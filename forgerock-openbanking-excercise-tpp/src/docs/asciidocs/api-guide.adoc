...
To get this client certificate setup, you need:

* register on the ForgeRock directory https://directory.integ-ob.forgerock.financial
* login to the directory
* create a software statement
* name your software statement
* as you are here, set the redirect uris to :
** https://localhost
** https://tpp.example.com:8080/web/exchange_code/aisp
** https://tpp.example.com:8080/web/exchange_code/pisp
* go to the keys tab and download the latest transport key in .pem and .key formats
* save then in the folder keys/ and rename the corresponding files by:
** transport.pem
** transport.key
...
Run the usual `mvn clean install`
You will see that it runs the functional tests. Feel free to explore them!
Bear in mind that the functional tests expect your software statement to not been on-board yet. Make sure that's the case.
By default, if you just created the software statement, it would be the case
...
This app is yelling for contribution! We prepared one exercise, which should introduce you nicely to this project.
The goal is to retrieve the PSU financial accounts and return them as json.
You will find a few 'TODO' that will indicate you where we expect the implementation to be done.
Submit a PR when you think you are ready and we will integrate your contribution into this sample app.
We build this app to show you what looks like a base for a TPP application but also give you a taste on what is Open Banking.
