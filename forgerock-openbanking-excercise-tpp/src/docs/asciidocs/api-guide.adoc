...
** http://localhost
** http://localhost:8080/web/exchange_code/aisp
** http://localhost:8080/web/exchange_code/pisp
...
NOTE: In the response is the aspspId. You should substitute this into any aspspId variable in the examples below
...
The result of the PSU interacting with their ASPSP is that you will receive an exchange code in the redirect URI. This
exchange code is used by the TPP to get the access token so that they can finally submit the payment.
